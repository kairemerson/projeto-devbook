...
api.interceptors.response.use(
    ...
    async (error) => {
        ...
        const canTryRefresh = config.url !== "/user/signin" &&
            error.response &&
            ...
            try {
                ...
                const response = await axios.post(
                    `${API_BASEURL}/user/refresh`, undefined,
                    {
                        headers: {
                ...
                localStorage.setItem(DEV_BOOLS_SESSION_KEY, JSON.stringify({...parsedSession, accessToken, refreshToken}))
                return api(config)
            } catch (error) {
                localStorage.clear()
